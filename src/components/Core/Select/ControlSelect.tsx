...
export const ControlSelect: FC<Props> = ({ items, value, handleChange }) => {
  return (
        ...
        {items &&
          items.map(item => (
            <MenuItem
              key={item.title}
              value={item.value}
              sx={{ fontSize: '16px', fontWeight: 400, lineHeight: '24px', fontFamily: 'Open Sans' }}
            >
              <Box mr={1} sx={{ color: '#82868B' }}>
                {item.id}
              </Box>
              <Box>{item.title}</Box>
            </MenuItem>
